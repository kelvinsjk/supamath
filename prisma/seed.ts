...
import { Polynomial, gcd, solveQuadratic } from 'mathlify';
	...
	for (const { b } of generateB(c)) {
		for (const { e } of generateE(d)) {
			for (const { canTake } of generateCanTake()) {
				...
				const poly = new Polynomial([A, B, C]).simplify();
				const [a0, a1, a2] = poly.coeffs;
				const bigCoeff =
					Math.max(Math.abs(a0.valueOf()), Math.abs(a1.valueOf()), Math.abs(a2.valueOf())) > 10;
				// 84.1% of the time we have surd solutions
				if (xType === 'frac' || !bigCoeff) {
					if (xType === 'frac') {
						k++;
					}
...
console.log(vars.length, k, vars.length - k, (k / vars.length).toPrecision(3));
