...
function case1Gen(case1: number,
	...
	if (case1===0){
		...
		return [math(`\\frac{1}{x \\ln x}`), ans, ans, '']
	}
	if (case1===1){
		const xA = `x^{${a1}}`;
	...
	const qn = math(`\\frac{${fPrimeSimplified}}{${fx}}`);
...
function case2Gen(case2: number, a2: number, b2: number, n: number): [string, string, string, string] {
	...
	// 0: ln x, 1: x^a + b, 2: trig
	if (case2===0){
		const qn = n===1 ? math(`\\frac{\\ln x}{x}`)
			: n>1 ? math(`\\frac{(\\ln x)^{${n}}}{x}`)
			: math(`\\frac{1}{x (\\ln x)^{${-n}}}`);
	...
	if (case2===1){
		...
		const qn = n > 0 ? math(`${fPrime} (${fx})^{${n}}`)
			: math(`\\frac{${fPrime}}{(${fx})^{${-n}}}`);
		let den = a2*(n+1);
		let multiple = 1;
		if (den < 0){
			multiple = -1;
